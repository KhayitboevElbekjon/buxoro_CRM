...
class Davomat(View):
    ...
    def post(self, request, pk):
        if request.user.is_authenticated:
            mavzu_id = request.POST.get('mavzu')
            talabalar = request.POST.getlist('davomat')
            if mavzu_id and talabalar:
                mavzu_fk = Mavzu.objects.get(id=mavzu_id),
                guruh_fk = Guruh.objects.get(guruh_nom=pk)
                asosiy_talabalar_safi_fk = Asosiy_talabalar_safi.objects.filter(guruh_fk__guruh_nom=pk),
                for talaba_id in talabalar:
                    talaba = Asosiy_talabalar_safi.objects.get(id=talaba_id)
                    Davomat.objects.create(
                        mavzu_fk=mavzu_fk,
                        guruh_fk=guruh_fk,
                        asosiy_talabalar_safi_fk=asosiy_talabalar_safi_fk,
                        darsga_qatnashmadi=False,
                        teacher_fk=Teacher_table.objects.get(user=request.user),
                        talaba_fk=talaba,
                    )
